...
class CrimeFragment : Fragment(), DatePickerFragment.Callbacks {
    ...
    override fun onStart() {
        ...
        photoView.apply {
            setOnClickListener {
                FullPictureViewFragment.newInstance(photoFile).apply {
                    show(this@CrimeFragment.requireFragmentManager(),"FullImageDialog")
                }
            }
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        when {
            ...
            requestCode == REQUEST_CONTACT && data != null -> {
                ...
                //указание, для каких полей запрос возвращает значение
                val queryFields = arrayOf(ContactsContract.Contacts.DISPLAY_NAME)
                //запрос похож на where
                val cursor = requireActivity().contentResolver.query(contactUri, queryFields, null, null, null)
                cursor?.use {
                    if (it.count == 0) {
                        return
                    }
                    //первый столбец первой строки данных - нужные данные
                    it.moveToFirst()
